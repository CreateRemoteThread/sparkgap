...
class AttackModel:
  def __init__(self):
    ...
    self.knownKey = 0x0
    self.knownLen = 0
    self.useKnownKey = False

  def loadOptions(self,om):
    if "knownKey" in om.keys() and "knownLen" in om.keys():
      self.knownKey = int(om["knownKey"],16)
      self.knownLen = int(om["knownLen"])
      print("OptionManager: using known key 0x%x with len %d" % (self.knownKey,self.knownLen))
      self.useKnownKey = True
  ...
  def genIVal(self,tnum,bnum,kguess):
    knownKey = self.knownKey # 0x0218 # 0x0218debf # orig is 0x0218debd
    knownKeyLen = self.knownLen # 16 # 32
    useKnownKey = self.useKnownKey # False
    ...
    if tnum in self.fragCache.keys():
      ...
      if useKnownKey:
        for i in range(0,len(knownKeyBitString)):
          (decr,dist1) = keeloqDecryptKeybitHD(decr,int(knownKeyBitString[i],2))
    ...
    for i in range(0,len(keyGuessBitString)):
      (decr,dist1) = keeloqDecryptKeybitHD(decr,int(keyGuessBitString[i],2))
    return dist1

...
# stole these from chipwhisperer
...
class AESHelper:
  def __init__(self,knownKey):
    print("AESHelper: Instatiating")
  ...
  # i stole this off newaetech/chipwhisperer
  def _shiftrow (self,row, shift):
  ...
  def shiftRows(self,state):
    for i in 1,2,3:
      state[i::4] = self._shiftrow(state[i::4],i)
    return state


  def _mixcolumn (self,column, inverse):
  ...
  def mixColumns (self,state, inverse):
    # Perform mix_column for each column in the state
    for i, j in (0, 4), (4, 8), (8, 12), (12, 16):
      state[i:j] = self._mixcolumn(state[i:j], inverse)
    return state
...
class AttackModel:
  ...
  def loadOptions(self,OptionManager):
    ...
    if "knownKey" in OptionManager.keys():
      ...
      self.cache = {}
      key = OptionManager["knownKey"]
      self.knownKey = [int(key[i:i+2],16) for i in range(0,len(key),2)]
      print(["%02x" % x for x in self.knownKey])
      if len(self.knownKey) != 16:
        print("KeyLength = %d, expected 16" % len(self.knownKey))
        sys.exit(0)
  ...
  def genIVal(self,tnum,bnum,kguess):
    ...
    return bin(self.genIValRaw(tnum,bnum,kguess) ^ self.pt[tnum,bnum]).count("1")
  ...
  def distinguisher(self,tnum,bnum,kguess):
    global HW_LUT
    return HW_LUT[self.genIValRaw(tnum,bnum,kguess) ^ self.pt[tnum,bnum]] >= 4

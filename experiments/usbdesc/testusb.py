...
#
# MODIFIED FOR USB GLITCHING
# DO NOT USE FOR NORMAL USB.
...
FGD_CONFIGURATION	=0x02	# Get device descriptor: Configuration
...
class GoodFETMAXUSB(GoodFET):
  ...
  def wreg(self,reg,value):
    ...
    self.writecmd(self.MAXUSBAPP,0x00,len(data),data);    
    return value;
  def wreg_trigger(self,reg,value):
    """Poke 8 bits into a register. set trigger high"""
    data=[(reg<<3)|2,value];
  ...
  def readbytes(self,reg,length):
    ...
    data=[(reg<<3)]+list(range(0,length));
    self.writecmd(self.MAXUSBAPP,0x00,len(data),data);
  ...
  def readbytesAS(self,reg,length):
    ...
    data=[(reg<<3)|1]+list(range(0,length));
    self.writecmd(self.MAXUSBAPP,0x00,len(data),data);
    toret=self.data[1:len(self.data)];
    ashex="";
    for foo in toret:

...
class GoodFET:
  ...
  def writecmd(self, app, verb, count=0, data=[]):
    ...
    self.serial_write_wrapper(chr(app));
    self.serial_write_wrapper(chr(verb));
    ...
    self.serial_write_wrapper(chr(count&0xFF));
    self.serial_write_wrapper(chr(count>>8));
    ...
    if count!=0:
      ...
      #print type(data);
      outstr=''.join(data);
      self.serial_write_wrapper(outstr);
  ...
  def serial_write_wrapper(self,data):
    jfc_fuck_unicode = []
    for i in range(0,len(data)):
      jfc_fuck_unicode += [ord(data[i])]
    self.serialport.write(bytes(jfc_fuck_unicode))
  ...
  def setBaud(self,baud):
    ...
    self.serial_write_wrapper(chr(0x00));
    self.serial_write_wrapper(chr(0x80));
    self.serial_write_wrapper(chr(1));
    self.serial_write_wrapper(chr(baud));

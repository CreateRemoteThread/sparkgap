...
class CaptureInterface():
  ...
  def init(self):
    ...
    if "capturecount" in self.config.keys():
      print("ps6000: overriding samplecount with capturecount")
      print("ps6000: will CAPTURE capturecount, will SAVE samplecount")  
    ...
    if self.config["pico_coupling"] == "AC":
      ...
      self.ps.runBlock()
      time.sleep(3.0)
    print("ps6000: got frequency %d Hz, %d max samples" % (freq,maxSamples))
    self.freq = freq
